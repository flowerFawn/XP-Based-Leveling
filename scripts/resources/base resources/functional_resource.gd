...
##This creates a raycast that will hit enemies with a (0, 0) target position, reconfigure it if you want it to hit a different group.
...
func get_raycast_colliders(raycast:RayCast2D) -> Array[Node2D]:
	var collider_array:Array[Node2D] = []
	while raycast.is_colliding():
		collider_array.append(raycast.get_collider())
